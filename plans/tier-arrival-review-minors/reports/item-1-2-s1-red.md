...
setup error, and ordered ahead of the `$TMPDIR` assertion so it is the one that
trips today.

**What `seen` contained** (captured via a temporary debug `cp`, reverted before
the final run above):
...
One line, under the tier's gitdir
(`.git/modules/gitlore-memory/modules/ddaanet`), none under `$TMPDIR` —
confirming today's `gitlore_adopt_repair_arrival` creates the scratch directory
inside the tier's gitdir via `mktemp -d "$gitdir/gitlore-repair.XXXXXX"`.
