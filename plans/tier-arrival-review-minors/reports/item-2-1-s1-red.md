...
The other 15 tests in the file, including the existing repair-publication cases
at :338, :376 and :404, pass unchanged.

## Which assertion fails, and why that is the defect

Both variants fail on their *last* assertion — the one checking that `aa`'s own
remote holds the commit memory's remote recorded as `aa`'s gitlink:
...
- the one-shot hook fired: `aa`'s remote `live` moved to D behind the push's
  back (`[ "$(git --git-dir=.../.bare-aa.git rev-parse live)" = "$d" ]` passes);
- the take, running while `bb` is processed (`tier 'bb' — fast-forwarded…` /
  ...
  (`tier 'aa' — fast-forwarded…`, `repaired aa's arrival…`), even though `aa`'s
  own loop iteration had already finished;
...
This is exactly the race Item 2.1 targets: the mid-loop take pass repairs a tier
whose own loop iteration already ran, but nothing pushes that repair to *that*
tier's remote — the removed-in-this-item retry push only covers the tier
currently being processed (`bb`), never a different tier the same take pass also
touched (`aa`). The message
`gitlore: tier 'aa' — the repair is committed in its local 'live', and this push publishes it.`
is printed and is false: this run does not publish it.
