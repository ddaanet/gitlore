...
1. **`preimage_file is unsuffixed with no agent id`** — PASS, by construction.
   ...
   `gitlore-index-preimage`. Today's `gitlore_index_preimage_file` takes only
   `$1` and ignores a second argument entirely, so both calls already return
   the unsuffixed path. Not a red case; reported as expected pass per the
...
2. **`preimage_file suffixes the agent id`** — RED, failed assertion.
   ...
   `$output` is the plain `.../gitlore-index-preimage`, which does not match
   the `*-agent-7` suffix pattern. This is a failed assertion against a value
   that ran cleanly, not a missing-symbol or syntax error.
...
assertion text quoted above are the RED run's, not the tree's. The slice's
shape is unchanged — see `item-2-1-s1-test-review.md` for the re-run.
