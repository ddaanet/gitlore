...
**Goal:** Add a `tests/evals/` suite that runs the gitlore memory-commit flow
through a real CC session and grades both pipeline compliance and commit-message
quality.

**Architecture:** Shell runner (`run-evals.sh`) reads JSON scenario files, calls
`setup.sh` helpers to build an ephemeral gitlore-installed repo, drives two
`claude --print` turns (edit + approve), fires a parent `git commit`, then runs
code-based outcome assertions followed by an LLM judge for commit message
quality. pass^k (k=5): every trial must pass.

**Tech Stack:** bash, bats (existing), jq, `claude` CLI (`--print`,
`--output-format json`, `--resume`), `ANTHROPIC_API_KEY`
...
- [ ] **Step 2: Run tests and confirm they fail with "setup.sh not found" or
      similar**
...
The runner has no separate bats test — it IS the test harness. Verify by running
it against the one scenario after implementation.
...
Also add `evals` to the `.PHONY` line at the top of the Makefile (update the
existing `.PHONY` declaration):
```makefile
