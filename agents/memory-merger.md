...
4. Judge each side's intent from its own diff. A conflict chunk carries three sections — `<<<<<<<` MINE, `|||||||` the base text both sides started from, `=======`, `>>>>>>>` THEIRS. The base is what tells an addition apart from a deletion: text present in MINE and absent in THEIRS was either added by MINE or deleted by THEIRS, and only the base section says which. A deliberate deletion is a decision to respect, not a gap to fill back in.
