# Memory entry points — decisions D16, D20

The two callable scripts a caller outside gitlore's process reaches through a
git-config key — `commit-memory.sh` and `push-memory.sh` — and the `push` skill
that drives the second. Each does with no parent commit or push in flight what a
git hook does inside one, through a body it shares with that hook; the hooks
themselves and the order they run in are in [git-hooks.md](git-hooks.md). The
approval gate a commit through either path carries out is in
...
rather than a mode of the hook, and why the push skill calls one of them
directly.
