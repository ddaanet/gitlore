...
- [~] **5. Add gitlore entry to `~/code/claude-plugins/.claude-plugin/marketplace.json`** — entry + README row added (mirrors `gitmoji`/`handoff` github-source shape); `claude plugin validate` exit 0 in both repos (gitlore has a benign pre-existing CLAUDE.md warning); committed (`9088b2a`). **Push BLOCKED:** auto-mode classifier denies pushing to `claude-plugins` as an unauthorized external repo. Needs explicit user authorization or a manual `git -C ~/code/claude-plugins push origin main`.
- [ ] **6. Outer-loop dogfood** in a fresh CC session (no `--plugin-dir`): `/plugin marketplace add ddaanet/claude-plugins` (idempotent), `/plugin marketplace update ddaanet`, `/plugin install gitlore@ddaanet`, repeat Step 3's divergence flow. Any difference from inner loop is a real bug. **Blocked on Step 5's push** + requires a fresh interactive session (cannot be run from this session).
