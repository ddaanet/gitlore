...
**D17 — Mechanism for tiered memory (FR15): nested global/org submodules composed into the project index**
...
Consequence: a tier's facts become reliably recall-reachable **iff their pointers appear in the root `MEMORY.md`**, regardless of where the bodies physically live. The design therefore keeps Anthropic's memory structure (index + files + agentic recall) and upgrades only gitlore's *generation* of the root index.
...
**Index generation — PostToolUse(Write|Edit), description derived from frontmatter.** The agent picks only *position* — the file's directory and where its pointer sits in that tier's own `MEMORY.md`. A `PostToolUse(Write|Edit)` hook, no-op unless the edited path is under the memory dir, derives the one-liner *description* from the file's frontmatter (frontmatter is the single source of truth → no index/frontmatter drift) and recomposes the **root** `MEMORY.md`: each tier's block emitted in precedence order (global tiers first), project's bare-path lines last. Root `MEMORY.md` thus has exactly two writers that never collide, because they own disjoint, path-prefix-partitioned line-sets — this PostToolUse recompose (on authoring) and SessionStart (on propagation); CC's native writer touches only the bare-path project lines. Because the block is keyed on path, the *pointer* is gitlore's to derive from where the file sits, so routing correctness reduces to "did the agent pick the right directory" — nothing depends on CC formatting a prefixed pointer.

*Distinct from the rejected "PostToolUse on every memory Write/Edit" (Rejected Alternatives).* That row rejects PostToolUse as a trigger for **commit preparation** (couples commit intent to individual edits; noisy). D17's PostToolUse does **index composition** — cheap, idempotent, no commit semantics — an orthogonal use of the same event.
...
*Status: design only — grounded and converged, but not yet implemented or planned. Promote to an FR and a plan when scheduled.*
