...
**Compose triggers and validation.** Composition runs at `SessionStart` (after the tier fast-forward, to surface propagated lines) and mid-session on **`PostToolUse` when `memory/.gitlore-tiers` changes** — the magic-file trigger that lets the agent edit the manifest, see the regenerated index, and adjust ordering within one session (the add-tier validate-and-reorder loop; SessionStart-only composition is ruled out by this story). The mid-session recompose **validates and is fail-safe**: it refuses — reporting on `systemMessage` (user) + `additionalContext` (agent) without clobbering the existing index — if the result would carry a **duplicate** pointer line, or if the manifest **lists a tier that is not present** (a stale or mistyped entry). A mounted tier *absent* from the manifest is not an error, only inactive (the asymmetry is deliberate: listed-but-absent = broken; present-but-unlisted = dormant). The mid-session trigger for *mirror-down of a freshly-authored tier line* — so it reaches the carrier before the FR11 commit rather than lagging a session — is settled when composition is built; the leading option is the same `PostToolUse` on memory writes, cheap and idempotent, and design-blessed as structural (not commit) semantics.

**Add and create — two scenarios, manifest edited last.** **Mount** (common — a repo joins an existing tier): `git submodule add <url> memory/<name>`, then list it. **Create** (rare — the first repo stands up `ddaanet`): init an empty module, seed its `MEMORY.md` + frontmatter `description:`, create and push its remote, *then* list it — the gap between "module exists" and "module self-describes" is invisible to composition precisely because it is not yet in the manifest. `/gitlore:add-tier <url>` performs the mount (a `--create <name>` path the creation flow); rarely used, but a real command beats hand-editing files by README directions. Because `git submodule add` and the memory-submodule commit are git mutations the agent cannot run under the FR11 gate/classifier, the command routes the git through the same trigger-file pattern as the FR11 commit path (agent writes intent; a hook runs git) — resolved when the command is built.

*Coverage, prune, and dedup were considered and dropped as premature.* **Coverage** (reconstruct a missing pointer line from a file's frontmatter) and **prune** (delete a bullet whose file is gone) both presuppose an answer to a question this design has *not* settled: whether the **file set** or the **index** is authoritative over a line's *presence*. Note the axis — the index is canonical for a line's *text* (settled empirically by the SPOT eval), but presence is separate and open. Deciding it well needs evidence we do not yet have (a log analysis of how presence actually drifts in real stores); the current hunch even leans *file-set-authoritative*, which would favour coverage/prune — so the point is not that they are wrong, but that baking either direction into a hook now decides by default what should be decided on evidence, later. Independently, each silently hardcodes a *semantic* call — was this deletion deliberate? A file with no pointer is either net-new (agent/CC writes the pointer on save) or a line the user removed on purpose (coverage would resurrect it); a bullet with no file may be the last recoverable record of a lost memory (prune would destroy it). That judgement belongs to the agent, not a hook, and neither drift was observed to bite. **Dedup** guarded duplicate path lines from a `merge=union` driver — but no such driver is used (see Conflicts below), and distinct per-index namespaces make cross-index duplication impossible, so it guards nothing in any slice. *When the presence-authority question is taken up (slice 3b or later, evidence permitting), coverage/prune return to the table on their own merits.*
...
| Recompose owns index-line presence: coverage (seed a missing pointer from frontmatter) + prune (drop a bullet whose file is gone) | Premature, not wrong. Both presuppose that the *file set* is authoritative over line *presence* — a question this design has not settled (the index is canonical for line *text*; presence is a separate, open axis whose direction needs log evidence we lack, and the hunch actually leans file-set-authoritative). Baking either into a hook now decides by default what should be decided on evidence, later. They also silently hardcode a *semantic* call (was a deletion deliberate?) that belongs to the agent, and neither drift was observed to bite. Built once (`SessionStart` recompose) and reverted 2026-07-17; revisit when the presence-authority question is taken up (D17). |
