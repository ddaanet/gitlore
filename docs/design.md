...
**Authoring-time bidirectional sync (Pre/Post) keeps the two surfaces in step.** A separate `PreToolUse` + `PostToolUse(Write|Edit)` pair keeps a file's index one-liner and its frontmatter `description` in sync: `PreToolUse` captures the pre-edit image, `PostToolUse` detects which surface's text the agent just changed and mirrors it to the other (only the after-the-dash hook ↔ `description`; never `name`/title, which is the `[[wikilink]]` slug and file identity). The tool boundary supplies the freshness signal the raw drift lacks — whichever surface just changed is, by construction, the fresh one. It deploys globally through the plugin hooks (no per-project work) and heals both drift directions on the next authoring edit. It is **complementary to, not a substitute for, the structural recompose**: the sync sees only in-session tool edits, so propagation (tier lines arriving via `SessionStart` ff, merges, or `/gitlore:resolve`) is invisible to it — precisely what the structural pass covers.

**One-time reconcile — after the sync, never before.** Pre-existing drift needs a one-time reconcile to establish the initial sync, and it **must run once the bidi sync is already deployed**: reconciling first and then continuing to edit without the sync in place just re-drifts the pairs and wastes the sweep. The reconcile is *semantic* (pick the correct one-liner per divergent file) and is **per-project** (each repo's store carries its own accrued drift) plus **once per shared tier**. Since the sync deploys everywhere automatically but the reconcile is a per-project cost that scales, it is **opportunistic, not mandated**: the sync heals each pair lazily on its next edit, and an explicit sweep (optionally a `/gitlore:reconcile` command, dogfooded here first) is worth running only where a stale **index** line bites — the harmful direction, since the index is the reliable lever; stale frontmatter is low-harm. Non-gitlore CC-memory stores are out of scope (no gitlore hooks, no sync).
...
*Status: design revised 2026-07-14 — grounded and converged, not yet implemented or planned. Implementation sequence when scheduled: (1) authoring-time bidi sync, (2) one-time reconcile (dogfood here first), (3) structural recompose + first nested tier. Promote to a plan when scheduled.*
