...
The mechanism is a subsystem rather than a single call: its decisions (**D26–D40 and D42–D44** — retrieval and routing, index composition, the authoring-time sync, and tier stores and merges) live in [docs/references/tiered-memory.md](references/tiered-memory.md), split out because the cluster runs to ~47 KB. Most of it is needed only when touching this subsystem; the tier-store decisions are the exception, since the commit and merge paths reach them from outside it (D42 from `pre-commit`, D43 from `/gitlore:merge`, D44 from every index merge). The branch model those three rest on is D41, with the rest of the merge decisions.
