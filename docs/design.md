...
**The wording is a single fragment, not a full sentence.** `reference/memory-approval-clause.txt` holds one mid-sentence fragment — "as one line per memory file — New, Update, Augment, Reduce, or Remove, its tier/slug, and a one-line summary of what changed" — read via `gitlore_memory_approval_clause()` (`scripts/lib/util.sh`) and interpolated into each call site's own surrounding sentence. The per-file kind is named rather than left to be inferred from prose, so the user scans the approval body instead of reading it. The four sites keep their distinct framing (a `PostToolUse` nudge, a pending-trigger retry, a sync-guard refusal, `resolve.sh`'s divergence-path refusal) — composing a shared fragment into different sentences, not centralizing the sentences themselves, which would have erased distinctions that exist on purpose.
