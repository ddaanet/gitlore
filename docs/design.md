...
**Index composition is structural, never text-derivation.** The recompose owns a line's *presence and placement*, never its text. A `PostToolUse(Write|Edit)` hook (no-op unless the edited path is under the memory dir) and `SessionStart` do only path-keyed structural work on the **root** `MEMORY.md`: (a) **coverage** — every memory file has a line; a *net-new* file with no line yet is seeded from its frontmatter (the only case with no curated text to preserve); (b) **prune** — drop orphaned lines whose file is gone; (c) **placement** — each tier's lines are emitted as a block in precedence order (global tiers first), the project's bare-path lines last, and a tier's lines are mirrored into that tier's own carrier (`memory/<tier>/MEMORY.md`) so they travel with the submodule and are spliced into a consuming repo's root index at `SessionStart`; (d) **dedup by path** — heals union-driver residue. The agent owns each line's *text*; the recompose never rewrites it. Root `MEMORY.md` writers are therefore disjoint by *aspect* — the agent owns text, the recompose owns presence/placement — not carved into rival line-sets, and CC's native pointer-writing is subsumed by the coverage step.

Slice 3a ships the recompose at **`SessionStart` only**, not `PostToolUse` — a cold-store pass, run after the live ff-merge and before the agent authors anything, cannot race the agent's own in-session index authoring (a mid-session dedup could drop a line the agent had just curated). Its three flat-store operations are coverage, prune, and dedup-by-path; **placement (c) and carrier mirroring are deferred to slice 3b**, where a nested tier first makes placement non-trivial. Coverage seeds a net-new line as `- [<title>](<path>) — <hook>` with `title` from frontmatter `name` (fallback: basename sans `.md`) and `hook` from `description` (fallback: title, so a description-less file still carries real words for retrieval). The recompose is idempotent and writes only on real drift; that write dirties `memory/` and rides the next parent commit like any memory change (the "float"). The `PostToolUse` trigger returns in 3b if sub-session healing proves necessary, with the anti-race handling that requires.
...
*Distinct from the rejected "PostToolUse on every memory Write/Edit" (Rejected Alternatives).* That row rejects PostToolUse as a trigger for **commit preparation** (couples commit intent to individual edits; noisy). D17's PostToolUse does **structural index composition** (coverage, prune, tier placement — never text derivation) — cheap, idempotent, no commit semantics — an orthogonal use of the same event.
...
**Conflicts on shared tiers.** Two repos inserting into a tier's `MEMORY.md` at the same position are resolved by a union driver (`**/MEMORY.md merge=union` in `.gitattributes`) — git takes both lines, exactly the "concatenate" resolution, no markers, no manual step. Any duplicate-pointer residue is normalized by the recompose's dedup-by-path on the next pass. No append-only constraint is imposed; conflicts are expected rare (the more global a tier, the more stable it presumably is).

*Status: design revised 2026-07-14 (sync corrected to one-way index→frontmatter 2026-07-15); SPOT empirically settled 2026-07-16 (eval over 528 transcripts / 8 projects: agents curate the index hook over frontmatter ~3:1, confirming the index one-liner as canonical). Implementation sequence: (1) authoring-time one-way sync (index→frontmatter) — **done**, destination-keyed with fill-if-empty for added lines (2026-07-16); (2) one-time reconcile — **dogfooded here 2026-07-17** (4/60 index lines stale; 2 also had stale bodies, so the reconcile re-verifies claims against current reality rather than blindly propagating body→index; no command built — a manual sweep sufficed); (3a) **structural recompose (coverage + prune + dedup), `SessionStart`-only — done 2026-07-17** (`scripts/lib/index-recompose.sh`; tier-block placement + carrier mirroring + the `PostToolUse` trigger deferred to 3b); (3b) nested-tier materialization + placement — next.*
...
| Append-only constraint on shared-tier indexes to avoid conflicts | Unnecessary. `**/MEMORY.md merge=union` concatenates same-position insertions with no markers, and the recompose dedups by path — positional conflicts resolve automatically without constraining where the agent may insert (D17). |
...
| Date | Change |
|------|--------|
| 2026-07-17 | **D17 slice 3a (structural recompose) shipped.** New `scripts/lib/index-recompose.sh` exposes `gitlore_recompose_index <mempath>`: dedup by path, prune orphaned lines, and seed a coverage pointer for every uncovered memory file (title from frontmatter `name`, hook from `description`, each with a fallback), owning each line's presence/placement but never a kept line's text. Idempotent, write-on-change, with an empty-store guard so a mis-resolved mempath can never wipe the index. Wired into `session-start.sh` after the live ff-merge (so it also heals union-driver residue), **`SessionStart`-only** to avoid racing the agent's in-session index edits — the `PostToolUse` recompose trigger and tier-block placement + carrier mirroring are deferred to 3b. Flat store only; the nested-tier `*.md` scan is 3b. Tests: `tests/index_recompose.bats` (11 cases: dedup, prune, coverage + fallbacks, placement, idempotence, empty-store, malformed-bullet preservation) + a `cc_hook_session_start.bats` case asserting SessionStart seeds an unindexed file and surfaces the healed-index notice. |
