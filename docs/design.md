...
**Materialization — nested submodules, framework reused.** Each tier is a git submodule mounted *inside* the project's memory submodule at a configurable path (`memory/lore`, `memory/ddaanet`, …; the Anthropic `team/` convention is just one possible name). It reuses the existing init, FR11 commit gate, and push-lockstep machinery — a submodule-within-a-submodule, more nesting of an already-solved problem (D11/D12 carry the linked-worktree and hook-env scar tissue).

**Routing — the agent picks the directory; no content classifier.** Which tier a new fact belongs to is a *generation* choice the authoring agent owns, guided by an addition to the standing SessionStart `additionalContext` (the D12 orientation block): portable facts → `memory/<tier>/`, project facts → `memory/`. The directory *is* the tier *is* the submodule, so the FR11 gate commits whichever submodule the file landed in — no shell logic inspects a fact's content to route it. This is consistent with D7 ("scripts decide"): that principle governs *detection/mechanics*, and where to file a fact you are already authoring is not detection. (The routing guidance rides `additionalContext` — model-only per D14, and observed not to inject under `--print` — so a missed guidance misfiles a global fact into the project tier: self-correctable, never corruption.)

**Index composition is placement, never text-derivation and never hygiene.** The composition owns a *tier* line's presence and placement *in the root index*, never its text and never the project's own lines. At `SessionStart`, each mounted tier's carrier (`memory/<tier>/MEMORY.md`) is spliced into the **root** `MEMORY.md` as a block in precedence order (global tiers first), the project's own bare-path lines last; a tier that is no longer mounted has its block removed; and root-level tier lines are mirrored back into that tier's carrier so they travel with the submodule into every consuming repo. That is the whole job. The agent — and CC's native pointer-writing — owns each line's *text* and its creation and deletion; composition only relocates tier blocks. Root `MEMORY.md` writers are therefore disjoint by *aspect*: the agent owns text and per-file presence, the composition owns tier-block placement.
...
*Status: design revised 2026-07-14 (sync corrected to one-way index→frontmatter 2026-07-15); SPOT empirically settled 2026-07-16 (eval over 528 transcripts / 8 projects: agents curate the index hook over frontmatter ~3:1, confirming the index one-liner as canonical). Implementation sequence: (1) authoring-time one-way sync (index→frontmatter) — **done**, destination-keyed with fill-if-empty for added lines (2026-07-16); (2) one-time reconcile — **dogfooded here 2026-07-17** (4/60 index lines stale; 2 also had stale bodies, so the reconcile re-verifies claims against current reality rather than blindly propagating body→index; no command built — a manual sweep sufficed); (3) tier composition — placement/splice of tier carrier blocks into the root index + carrier mirroring, landing with the first nested tier — **next** (a first `SessionStart`-only recompose that also carried coverage/prune/dedup was built and reverted 2026-07-17: those three operations either presuppose an unsettled presence-authority decision or guard a non-existent union driver — see the composition and Conflicts paragraphs above).*
