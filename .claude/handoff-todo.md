...
- `/ddaa:preflight`.
- Release — only after preflight. 0.5.0 consumers still run the `sh -c` sentinel replay and `abort-then-retry`, their test suite and `lint-shell.sh` do not run on macOS, and a store with no root index is not repaired at session start.
